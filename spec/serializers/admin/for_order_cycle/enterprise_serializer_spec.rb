describe Api::Admin::ForOrderCycle::EnterpriseSerializer do
  ...
  let!(:non_inventory_product) { create(:base_product, supplier: enterprise) }
  let!(:non_inventory_variant)  { non_inventory_product.variants.first }
  let!(:inventory_product)  { create(:base_product, supplier: enterprise) }
  let!(:inventory_variant)  { inventory_product.variants.first }
  let!(:deleted_product)    { create(:base_product, supplier: enterprise, deleted_at: 24.hours.ago ) }
